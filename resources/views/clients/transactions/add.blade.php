                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{__('translation.New_Transaction')}}</h3>
                            </div>
                            <div class="col-4 text-right">
                                <a href="{{ route('clients.show', $client) }}" class="btn btn-sm btn-primary">{{__('translation.Back_to_Client')}}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('transactions.store') }}" autocomplete="off">
                            ...
                            <h6 class="heading-small text-muted mb-4">{{__('translation.Transaction_Information')}}</h6>
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('type') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-method">{{__('translation.Transaction_Type')}}</label>
                                ...
                                <div class="form-group{{ $errors->has('payment_method_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-method">{{__('translation.Payment_Method')}}</label>
                                ...
                                <div class="form-group{{ $errors->has('amount') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="number" step=".01" name="amount" id="input-amount" class="form-control form-control-alternative"
                                           placeholder="{{__('translation.Amount')}}" value="{{ old('amount') }}" required>
                                    @include('alerts.feedback', ['field' => 'amount'])

                                </div>

                                <div class="form-group{{ $errors->has('reference') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-reference">{{__('translation.Reference')}}</label>
                                    <input type="text" name="reference" id="input-reference" class="form-control form-control-alternative{{ $errors->has('reference') ? ' is-invalid' : '' }}" placeholder="{{__('translation.Reference')}}" value="{{ old('reference') }}">
