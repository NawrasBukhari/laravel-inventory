@extends('layouts.app', ['page' => 'List of Products', 'pageSlug' => 'kenzhekhan', 'section' => 'inventory'])
        ...
        <div class="col-md-12">
            <div class="card ">
                <div class="card-header">
                    <div class="row">
                        <div class="col-8">
                            <h4 class="card-title">{{__('translation.kenzhekhan')}}</h4>
                        </div>
                        <div class="col-4 text-right">
                            @foreach($kenzhekhan as $p)
                                @if($p->id == 1)
                            <a href="{{ route('export2') }}" class="btn btn-sm btn-success">{{__('translation.Download')}} <i class="tim-icons icon-cloud-download-93"></i></a>
                                @endif
                            @endforeach
                            <a href="{{ route('kenzhekhan.create') }}" class="btn btn-sm btn-primary">{{__('translation.New_Product')}}</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            @foreach ($kenzhekhan as $product)
                                <tr>
                                    ...
                                    <td class="td-actions text-right">
                                        <a href="{{ route('kenzhekhan.show', $product) }}" class="btn btn-link" data-toggle="tooltip" data-placement="bottom" title="{{__('translation.More_Details')}}">
